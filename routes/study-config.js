...
const create_app_data = async (dataType, dsid, metaDataJSON) => {
  const query = `SELECT fn_create_app_data($1, $2, $3) as app_data_id;`;
  const rows = await db.query(query, [dataType, dsid, metaDataJSON]);
  return rows[0].app_data_id;
}
...
router.post('/', authenticateToken, uploadFileds, async (req, res) => {
  try {
    ...
    // -- pull study_module_status
    const rows_sms = await db.query('SELECT * FROM fn_get_study_module_status($1);', [study_id]);
    const user_input = rows_sms.find((row) => row.module_name === 'user-input');
    const config_exists = user_input && user_input.study_module_status !== 'waiting';

    if (config_exists)
      return res.status(500).json({studyConfigId: -1, message: "Study Config already exists! Use PUT to update."});
